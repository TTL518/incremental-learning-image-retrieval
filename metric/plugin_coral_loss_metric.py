...
class CoralLossPluginMetric(GenericPluginMetric[float]):
    def __init__(self, reset_at, emit_at, mode):
        self._loss = Loss()
        super(CoralLossPluginMetric, self).__init__(
    ...
    def update(self, strategy):
        if(strategy.plugins[0].prev_model_frozen != None):
            ...
            self._loss.update(strategy.plugins[0].coralLoss,
                            patterns=len(strategy.mb_y), task_label=task_label)

class MinibatchCoralLoss(CoralLossPluginMetric):
    """
    ...
    def __init__(self):
        ...
        """
        super(MinibatchCoralLoss, self).__init__(
            reset_at='iteration', emit_at='iteration', mode='train')

    def __str__(self):
        return "CoralLoss_MB"

class EpochCoralLoss(CoralLossPluginMetric):
    """
    ...
    def __init__(self):
        ...
        super(EpochCoralLoss, self).__init__(
            reset_at='epoch', emit_at='epoch', mode='train')

    def __str__(self):
        return "Coral_Loss_Epoch"